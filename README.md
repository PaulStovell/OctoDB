...
Since read sessions use in-memory snapshots, assuming you only perform one commit per second, you should be able to perform millions of read operations between those commits. And assuming the commits only touch a few dozen documents at once, updating the in-memory snapshot is also very quick. 
